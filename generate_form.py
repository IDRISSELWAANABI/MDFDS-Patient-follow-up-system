...
TO_EMAIL = 'drisselwaanabi02@gmail.com'

# Directory for storing patient responses
DATA_DIR = "patient_responses"
...
questions = [
    {"type": "short_text", "title": "How are you feeling today?"},
    {"type": "yes_no", "title": "Do you have a fever?"},
    {"type": "number", "title": "On a scale of 1-10, how is your pain level?"},
    {"type": "multiple_choice", "title": "Which symptoms are you experiencing?", 
     "properties": {
         "choices": [
             {"label": "Headache"},
             {"label": "Nausea"},
...
def save_response(response_data):
    ...
    filename = f"{DATA_DIR}/patient_{response_data['response_id']}.json"
    with open(filename, 'w') as f:
        json.dump(response_data, f, indent=4)
    print(f"Data saved to {filename}")

def send_email(form_url, form_id):
    ...
    <p><a href="{form_url}" target="_blank">Complete Patient Health Assessment</a></p>
...
def process_response(item, question_mapping):
    ...
    for answer in raw_answers:
        # Add error handling to safely extract field ID
        try:
...
def get_question_mapping(form_id):
...
def check_for_new_responses(form_id):
    ...
    for item in items:
        ...
        try:
            patient_response = process_response(item, question_mapping)
            
            save_response(patient_response)
    ...
    if not new_responses_found and len(processed_responses) > 0:
...
def main():
    # Create the Typeform
    patient_form_url, form_id = create_form()
    
    print("Patient Form System Ready!")
    print(f"Form URL: {patient_form_url}")
    
    # Send email with form URL
    email_sent = send_email(patient_form_url, form_id)
    if email_sent:
        print(f"Email with form URL sent successfully to {TO_EMAIL}")
    else:
    ...
    try:
        while True:
            ...
            check_for_new_responses(form_id)
            print(f"Next check in 30 seconds. Press Ctrl+C to exit.")
            time.sleep(30)
